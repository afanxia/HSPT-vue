import { asyncRouterMap, constantRouterMap, initMenu } from '@/router/index'
  ...
  actions: {
    GenerateRoutes({ commit }, userInfo) {
      ...
      return new Promise(resolve => {
        ...
        let tmpRouter = initMenu(menus)
        if (tmpRouter.length > 0) {
          tmpRouter = tmpRouter.concat(asyncRouterMap)
        }
        // 声明 该角色可用的路由
        let accessedRouters
        if (role === '系统管理员') {
